...
namespace Engine {
	namespace Math {
		Matrix &Matrix::InverseSSE(Matrix &o_out) const
		{
			__m128 temp = { 0.0f };
			...
			temp = _mm_shuffle_ps(temp, temp, 0xB1);
			minor2 = _mm_sub_ps(minor1, _mm_mul_ps(row2, temp));
			minor3 = _mm_add_ps(_mm_mul_ps(row1, temp), minor2);
			temp = _mm_shuffle_ps(temp, temp, 0x4E);
			...
			_mm_storeh_pi(reinterpret_cast<__m64 *>(&o_out.m_11), minor0);

			minor1 = _mm_mul_ps(det, minor1);
			_mm_storel_pi(reinterpret_cast<__m64 *>(&o_out.m_11), minor1);
			_mm_storeh_pi(reinterpret_cast<__m64 *>(&o_out.m_11), minor1);

			minor2 = _mm_mul_ps(det, minor2);
			_mm_storel_pi(reinterpret_cast<__m64 *>(&o_out.m_11), minor2);
			_mm_storeh_pi(reinterpret_cast<__m64 *>(&o_out.m_11), minor2);

			minor3 = _mm_mul_ps(det, minor3);
			_mm_storel_pi(reinterpret_cast<__m64 *>(&o_out.m_11), minor3);
			_mm_storeh_pi(reinterpret_cast<__m64 *>(&o_out.m_11), minor3);

			return o_out;
		}

		Matrix &Matrix::MultiplySSE(const Matrix &i_other, Matrix &o_out) const
		{
