		...
		while (!bDone)
		{
			...
			else if (LQResult == WAIT_OBJECT_0 + 1)
			{
				...
				if (pLoadTask)
				{
					...
					{
						DEBUG_CONSOLE("Error Opening File %s : %d", pLoadTask, GetLastError());
						delete pLoadTask;
					}
					else
					{
						...
						{
							DEBUG_CONSOLE("Failed reading %s : %d", pFileName, GetLastError());
							delete pLoadTask;
						}
						else
						{
							...
							{
								DEBUG_CONSOLE("Failed waiting on File I/O %s : %d", pFileName, GetLastError());
								delete pLoadTask;
							}
						}
					}
